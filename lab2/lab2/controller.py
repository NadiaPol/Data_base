...
class ControllerShop(object):
    ...
    def update_shop(self, shop):
        ...
        newShop = Check.updateShop(shop, [older[0].ShopId, older[0].ShopName, older[0].ShopStreet])
        check = self.model.update_shop(shopId, newShop['name'], newShop['street'])
        if check:
            print(older[0].ShopName, older[0].ShopStreet)
            self.view.display_shop_updated(shopId, newShop['name'], newShop['street'])
...
class ControllerCustomer(object):
    ...
    def update_customer(self, customer):
        ...
        if check:
            self.view.display_customer_updated(customerId, newCustomer['name'], newCustomer['phone'],
                                               newCustomer['sex'])
...
class ControllerItem(object):
    ...
    def update_item(self, item):
        ...
        item = Check.updateItem(item, [older[0].ItemId, older[0].ItemName, older[0].ItemPrice, older[0].ItemQuantity,
                                       older[0].ItemColor, older[0].ItemMaterial, older[0].ItemDescriptions])
...
class ControllerOrder(object):
    ...
    def update_order(self, order):
        ...
        newOrder = Check.updateOrder(order, [older[0].OrderId, older[0].CustomerId, older[0].ItemId,
